...
- **显式工具启用** - 聊天页工具列表由后端 `/api/v1/tools` 动态下发；只有本轮显式勾选的工具才会参与回答，未选择时不会隐式启用默认工具
...
说明：
- `/api/v1/tools` 返回当前可展示的工具目录，包含 `name`、`display_name`、`description`、`category`
- `POST /api/v1/agent/*` 中的 `tools` 表示“本次请求显式启用的工具列表”；传空数组或省略时表示本轮不启用工具
...
- 聊天页工具下拉选项由 `/api/v1/tools` 动态加载，并使用后端返回的 `display_name` 作为展示名称
