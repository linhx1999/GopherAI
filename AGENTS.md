...
- Controller 层由 `ChatHandler` 先创建事件 channel，再使用 `go handleStreamRequest(...)` 转发 service 事件，最后统一调用 Gin `c.Stream(...)` + `c.SSEvent("message", payload)` 输出 SSE；channel 关闭时追加 `data: [DONE]`
...
- HTTP SSE 由 Controller 先创建事件 channel，再用 goroutine 写入，最后统一使用 Gin `c.Stream` 逐条输出；channel 关闭时追加 `data: [DONE]`，其余 `data` 中的 JSON 结构保持不变
