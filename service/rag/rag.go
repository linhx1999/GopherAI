...
// IndexFile 索引指定文件
func (s *RAGService) IndexFile(fileID string, userRefID uint) error {
	...
	// 2. 更新状态为 indexing
	if err := file.UpdateIndexStatus(fileRecord.ID, fileModel.IndexStatusIndexing, "开始索引"); err != nil {
	...
	if err != nil {
		...
		file.UpdateIndexStatus(fileRecord.ID, fileModel.IndexStatusFailed, errMsg)
	...
	// 7. 更新状态为 indexed
	if err := file.UpdateIndexStatus(fileRecord.ID, fileModel.IndexStatusIndexed, "索引完成"); err != nil {
		log.Printf("Failed to update index status to indexed: %v", err)
	}

	log.Printf("File indexed successfully: fileID=%s, Name=%s", fileID, fileRecord.FileName)
	return nil
}

// DeleteIndex 删除文件索引
func (s *RAGService) DeleteIndex(fileID string, userRefID uint) error {
	ctx := context.Background()
	...
	// 更新状态为 pending
	if err := file.UpdateIndexStatus(fileRecord.ID, fileModel.IndexStatusPending, "索引已删除"); err != nil {
