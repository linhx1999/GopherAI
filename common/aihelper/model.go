...
type AliRAGModel struct {
	...
	fileID   uint   // 指定要检索的文件 ID
}

func NewAliRAGModel(ctx context.Context, username string, fileID uint) (*AliRAGModel, error) {
	key := os.Getenv("OPENAI_API_KEY")
...
func (o *AliRAGModel) GenerateResponse(ctx context.Context, messages []*schema.Message) (*schema.Message, error) {
	// 1. 创建 RAG 查询器（使用文件 ID）
	ragQuery, err := rag.NewRAGQuery(ctx, o.fileID)
	if err != nil {
		log.Printf("Failed to create RAG query (file may not be indexed): %v", err)
		// 如果文件未索引，直接使用原始问题
		resp, err := o.llm.Generate(ctx, messages)
		if err != nil {
